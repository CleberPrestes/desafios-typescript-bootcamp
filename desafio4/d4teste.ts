...
class HttpClient {
  static async get<T>({ url, method, body = null }: RetornoGet) {
...
async function criarRequestToken() {
  let result = await HttpClient.get<TokenGet>({
...
async function logar() {
  let result = await HttpClient.get<AutenticacaoDeLogin>({
...
async function criarSessao() {
  let result = await HttpClient.get<SessaoGet>({
...
async function procurarFilme(query: string): Promise<RespostaDeFilme> {
  query = encodeURI(query)
  console.log(query)
  let result = await HttpClient.get<RespostaDeFilme>({
...
async function criarLista(nomeDaLista: string, descricao: string) {
  let result = await HttpClient.get<ListaCriada>({
...
async function adicionarFilmeNaLista(filmeId: string, listaId: string) {
  let result = await HttpClient.get<AdicionarFilmeParaLista> ({
...
async function pegarLista() {
  let result = await HttpClient.get<ListaGet>({
